...
class ResNet(nn.Module):
    def __init__(self, block, layers, classifier=None, num_classes=None,
                 dropout=0.0, global_pool=True, require_flatten=True):
    ...
    def forward(self, x):
        x = self.embed(x)
        if self.require_flatten == True and num_classes is not None:
            x = self.cls_fn(x)
        return x
...
def resnet18_film(pretrained=False, pretrained_model_path=None, **kwargs):
    """
